...
func AssetDir(workingFolder string) ([]string, error) {
	// Look for an "assets" subdirectory in the working folder.
	assetsPath := filepath.Join(workingFolder, "assets")
	info, err := os.Stat(assetsPath)
	if err != nil {
		if os.IsNotExist(err) {
			// No assets directory -> return nil,nil so caller can disable GUI.
			return nil, nil
		}
		return nil, err
	}
	if !info.IsDir() {
		// Not a directory -> treat as no assets.
		return nil, nil
	}
